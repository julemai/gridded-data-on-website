...
Example code on how to display (large) gridded data (TIFF files) on a
website. 

## TiTiler Setup
This section discusses how to setup titiler, the main python module
used to allow for dynamic tiling.
...
In the same file location as the html file which will display the grid
dataset, create a main.py file that contains the following code: 
```
from titiler.application.main import app
```

#### 3. Run uvicorn
In a python command prompt, run the following while in the same
directory as the html file which will display the grid dataset:  
```
uvicorn titiler.application.main:app --host 0.0.0.0 --port 8000 --reload --proxy-headers --header "Access-Control-Allow-Origin:*"
```
To see if it is working, go to http://127.0.0.1:8000/. It should
display the TiTiler home page.  
...
There are two main manadtory steps for getting from a typical Geotiff
file to a coloured Cloud Optimized Geotiff (COG) file, and one
optional step. A file that underwent these steps and is used to be
displayed on the website is provided as `data/example_cog.tif`.

### 00. Setting nodata values and/or statistics
Nodata values need to be set if you wish them not to be displayed on
the website, i.e. will be transparent. The example code is provided as
`supplementary_files/00_NoData_Stats.py `.
...
This step converts the GeoTiff file into a cloud-optimized GeoTiff
(COG).
...
- Based off this gdal command: 
	...
	- `cog_file` is the output file as an absolute path; must be a new
      file; input file can not be overwritten
	...
	- For more information on `gdal_translate` see documentation [here](https://gdal.org/en/stable/programs/gdal_translate.html).

### 02. Converting COG to Coloured COG

This step converts the actual values of the GeoTiff into color
values that will be displayed for each gridcell on the website. 
...
- Based off this gdal command:
	...
	- `cog_file` is the filename of the file generated in step 01. 
	- `colour_txt_file` is a .txt file that contains value and RGB
      infomation. An example file can be found under
      `supplementary_files/02a_colours.txt` and more information found
      below.  
	- `colour_file` is the name of the outputed coloured COG
      file. This is the file that will be displayed on the website
	...
	- `-alpha` makes any NoData values transparent and uses the last
      ...
      information below. 
	- For more information on `gdaldem color-relief` see documentation
      [here](https://gdal.org/en/stable/programs/gdaldem.html#color-relief).
...
- Colour_txt_file information
	- For example: 
		- ```
		  ...
		  nv 0 0 0 0
		  ```
	- Each row in the text file represents `"value red green blue
      [alpha]"`. The alpha column will have significance when `-alpha`
	...
	- The final row `"nv 0 0 0 0"` indicates "no value" (nv) points
	  should be transparent  
	  
- *IMPORTANT:* In the new file this step creates, the values of all
  the pixels will represent their RGBA values and will lose their
  orignal value. 

## Displaying On Website Using Leaflet
Examples of the html and javascript required to display the gridded
